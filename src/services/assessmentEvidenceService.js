...
import { AssessmentEvidence } from "../models/assessmentEvidenceModel.js";
...
const extractTextFromBuffer = async (file) => {
  try {
    ...
    console.error("[assessment-evidence] extractText error", err.message);
  }
  return "";
};

export const AssessmentEvidenceService = {
  async createFromUpload({ file, uploaderId, uploaderRole, assessmentId, tenantId, linkedControlIds = [], linkedQuestionIds = [] }) {
  ...
  async issueViewToken({ evidenceId, viewerId, tenantId, jwtSign }) {
    const evidence = await AssessmentEvidence.findOne({ _id: evidenceId, tenantId, status: "ready" });
  ...
  async listByAssessment({ assessmentId, tenantId }) {
    return AssessmentEvidence.find({ assessmentId, tenantId }).sort({ createdAt: -1 }).lean();
  },
};
