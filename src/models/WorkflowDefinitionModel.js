...
// Universal workflow definition — replaces hardcoded auditPhases.js
...
const WorkflowDefinitionSchema = new mongoose.Schema({
  workflowKey: {
    ...
    // e.g. 'gmp_pharma_audit', 'organic_farming_coc', 'forest_coc'
  },
  displayName:   { type: String, required: true },
  ...
  partyLabel:   { type: String, required: true },   // "Supplier", "Farm", "Seller"
  subjectLabel: { type: String, required: true },   // "Drug Product", "Crop Lot"
  phases:       { type: [PhaseSchema], required: true },
  standardsLibrary: [{ type: String }],             // e.g. ['ICH_Q7','CFR_21_211']
  reportTemplateKey: { type: String },
  vocabularyDefaults: { type: VocabularyDefaultsSchema, default: () => ({}) },
  industryTags:  [{ type: String }],                // e.g. ['pharma','gmp','fda']
  isBuiltIn:     { type: Boolean, default: false }, // true = platform-shipped template
  isActive:      { type: Boolean, default: true },
  tenantId: {
    type: mongoose.Schema.Types.ObjectId, ref: 'Tenant',
    default: null, // null = platform-wide; set for tenant-custom definitions
  },
}, {
  timestamps: true,
  collection: 'workflow_definitions',
